...
MSYS_NO_PATHCONV=1 docker run -it --rm \
  ...
  snakemake/snakemake:v7.32.4 bash -c "curl -sSL https://download.docker.com/linux/static/stable/x86_64/docker-24.0.9.tgz | tar -xzC /tmp/ && mv /tmp/docker/docker /usr/bin/ && ./setup_matrix.sh && snakemake --cores 30 -s workflow/Snakefile"
